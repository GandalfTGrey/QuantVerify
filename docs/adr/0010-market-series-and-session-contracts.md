...
2. `CalendarArtifactRef` 绑定 calendar id/version、IANA timezone、来源/version、完整内容 hash 和版本化 session-label policy。`SessionSchedule` 绑定请求日期范围、该 artifact 和精确 session 集合。生产调用方只能使用来自已验证 calendar artifact 的 schedule，禁止从待验证 bars 反向自举 schedule。
...
9. 派生周期同时携带完整 expected schedule、截止时实际 constituent schedule、与 constituents 一一对应的 availability 和 `cutoff_at`。`complete` 是两组实际 session 对象完全相等的派生属性，调用方不能直接写入；相同数量但日期或时间不同不能伪装完整。
...
11. `period_bar_id` 绑定 descriptor、实际/预期 schedule identity、availability watermark 输入、cutoff 和完整 OHLCV；不同派生输出不能只因 transformation recipe 相同而共享内容 identity。
